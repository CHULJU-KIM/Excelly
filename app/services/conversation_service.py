...
class ConversationService:
    ...
    async def analyze_question_for_clarification(self, question: str, context: str = "") -> QuestionClassification:
        ...
        try:
            ...
            # 질문에 즉시 답변 키워드가 포함되어 있으면 즉시 답변
            question_lower = question.lower()
            has_immediate_keywords = any(keyword.lower() in question_lower for keyword in immediate_answer_keywords)
            
            # 완전히 모호한 경우만 명확화 필요
            vague_keywords = ["정리하고 싶어요", "자동화하고 싶어요", "분석하고 싶어요", "개선하고 싶어요"]
            is_vague = any(keyword in question for keyword in vague_keywords)
            
            # 판단 로직
            if has_immediate_keywords or not is_vague:
