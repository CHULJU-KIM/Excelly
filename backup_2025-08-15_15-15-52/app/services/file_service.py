...
class FileService:
    ...
    def get_file_summary(self, file_content: bytes, filename: str, sheet_name: Optional[str] = None) -> str:
        ...
        try:
            ...
            summary += f"📊 총 시트 수: {len(analysis_result.sheets)}개\n\n"
            ...
            if sheet_name and sheet_name != "all_sheets":
                ...
                try:
                    ...
                    df = pd.read_excel(xls, sheet_name=sheet_name)
                    summary += f"\n📈 시트 구조:\n"
                    summary += f"- 행 수: {len(df)}개\n"
                    summary += f"- 열 수: {len(df.columns)}개\n"
                    summary += f"- 열 이름: {list(df.columns)}\n"
                    
                    # 데이터 타입 정보
                    summary += f"\n🔍 데이터 타입:\n"
                    for col in df.columns[:5]:  # 처음 5개 열만
                        summary += f"- {col}: {df[col].dtype}\n"
                    
                except Exception as e:
                    summary += f"[⚠️ 시트 데이터 추출 오류: {str(e)}]\n"
                ...
                try:
                    ...
                    summary += f"[⚠️ 전체 시트 데이터 추출 오류: {str(e)}]\n"
            
            else:
                # 시트가 선택되지 않은 경우
                summary += f"[💡 시트를 선택하지 않았습니다]\n"
                summary += f"위의 시트 목록에서 분석할 시트를 선택해주세요.\n"
                summary += f"선택된 시트의 데이터 구조와 내용을 자세히 분석해드립니다.\n"
